...
import { CellProps, Column, SimpleColumn, gutterColumnWidth } from '../types'
...
export const useColumns = <T extends any>(
  ...
  >(
    columns.map((column) => {
      ...
      return { id: column.columnData.key, width: width }
    })
  )
    ...
    useMemo<Column<T, any, any>[]>(() => {
      ...
      const partialColumns: Partial<Column<T, any, any>>[] = [
        gutterColumn === false
          ...
          : {
              ...gutterColumn,
              width: gutterColumn?.width ?? `0 0 ${gutterColumnWidth}px`,
              minWidth: gutterColumn?.minWidth ?? gutterColumnWidth,
      ...
      return partialColumns.map<Column<T, any, any>>((column) => {
        ...
        const col = columnsWidth?.find((x) => x.id === column?.columnData?.key)
        if (col && col.width) {
